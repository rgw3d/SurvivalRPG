...
public class GenerateMap : MonoBehaviour {
	...
	private List<Vector2> initRoomNodes(){
		...
		for (int i = 0; i < roomNumber && tries<maxTries; i++){
			...
			foreach(Vector2 node in nodes){
				if (Mathf.Pow(Mathf.Pow(node.x - pos.x, 2) + Mathf.Pow(node.y - pos.y, 2), 0.5f) < minimumDistanceBetweenRooms){
					tries++;
					i--;
					addNode = false;
					break;
	...
	void createRooms(List<Vector2> positions) {
		...
		//you can see the process for this.  if it only two directions, then it is a corner, and a room can be created with those parameters
